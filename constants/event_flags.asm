...
;Bronze 2 Events
	const EVENT_RED_STONE_PLACED
	const EVENT_BLUE_STONE_PLACED
	const EVENT_BROWN_STONE_PLACED
	const EVENT_GREEN_STONE_PLACED
	const EVENT_ACRE_QUESTION_1 
	const EVENT_ACRE_QUESTION_2
	const EVENT_ACRE_QUESTION_3
	const EVENT_ACRE_HOLE
	const EVENT_ACRE_SAVE_BOY
	const EVENT_ACRE_FATHER
	const EVENT_ACRE_FOREST_LOOKING_DAD
	const EVENT_RED_STONE_PLACED_ON_ROCK   ;Turn this on at game start
	const EVENT_BLUE_STONE_PLACED_ON_ROCK  ;Turn this on at game start
	const EVENT_BROWN_STONE_PLACED_ON_ROCK ;Turn this on at game start
	const EVENT_GREEN_STONE_PLACED_ON_ROCK ;Turn this on at game start
	const EVENT_FOREST_QUIZ_MASTER_IN_GATE
	const EVENT_STUDIO_ROCKETS
	const EVENT_STUDIO_ROCKETS_WALK_IN
	const EVENT_STUDIO_SCARLET
	const EVENT_STUDIO_BLOCKER_MAN
	const EVENT_STUDIO_GAVE_STONE
	const EVENT_PASS_EXPIRED
	const EVENT_FOUGHT_AERODACTYL
	const EVENT_SHOPPING_STARTED
	const EVENT_SHOPPING_COMPLETE
	const EVENT_KOA_PICKING_POKEMON_ACVIATED
	const EVENT_KOA_POKEMON_TAKEN
	const EVENT_TOOK_CHIKORITA
	const EVENT_TOOK_CYNDAQUIL
	const EVENT_TOOK_TOTODILE
	const EVENT_SECRET_SWITCH_LADY_TALKED_TO
	const EVENT_SECRET_SWITCH1
	const EVENT_SECRET_SWITCH2
	const EVENT_SECRET_SWITCH3
	const EVENT_SECRET_SWITCH4
	const EVENT_SECRET_SWITCH_LADY_EVENT_OVER
	const EVENT_CELADON_LEADER_OFF       ;Turn this on at game start
	const EVENT_CELADON_TEMPORARY_LEADER ;Turn this on at game start
	const EVENT_CELADON_FAKE_LEADER_TALKED_TO
	const EVENT_TALKED_TO_CELADON_LEADER
	const EVENT_ALREADY_HAD_FIGHT_LEADER
	const EVENT_TALKED_TO_LEADER_IN_HOUSE_ONCE
	const EVENT_LEADER_IN_HOUSE_OFF
	const EVENT_TEMP1
	const EVENT_TEMP2
	const EVENT_TEMP3
	const EVENT_TEMP4
